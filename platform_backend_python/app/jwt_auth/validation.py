...
from users.schemas import ReadUserSchema, LoginUserSchema
...
async def validate_auth_user(user_data: LoginUserSchema,
    ...
    async def fetch_user_by_username(username: str, session_: AsyncSession = session) -> ReadUserSchema:
        user_ = await get_user(username=username, session=session_)
        if user_ is None:
            raise unauthed_exception
        return user_

    user = await fetch_user_by_username(user_data.username, session)
